...
comm_ts = 0
#WFBP
def WFBP(comm_ag_list, comm_rs_list, fw_ops, bw_ops):
    t_fw = 0
    for comp in fw_ops :
        t_fw += comp.total_comptime 
    print(t_fw)
    ts_b = 0
    idx = 0
    comm_rs_list.reverse()
    print(bw_ops)
    ...
    for comp in bw_ops: 
        ...
        if(idx == 0):
            ts_c = ts_b + comp.total_comptime
            ts_b = ts_b + comp.total_comptime
        else: 
            ts_c = max(ts_c + comm_rs_list[idx-1].time, ts_b+comp.total_comptime )
            ts_b  = ts_b+comp.total_comptime 
        idx += 1
    ...
    for comp in fw_ops:
        if(idx == 0):
            ts_cf = ts_cf + comm_ag_list[idx].time
            ts_f  = ts_f+ comm_ag_list[idx].time
        else:
            ts_f = max(ts_cf + comm_ag_list[idx].time, ts_f + comp.total_comptime)
            ts_cf =ts_cf + comm_ag_list[idx].time
        idx += 1
    return comm_rs_list[-1].time +  comm_ag_list[-1].time + ts_c + ts_cf
...
#a = WFBP(comm_ag_list, comm_rs_list, forward_ops, backward_ops)
#print("!!!!")
#print(a)
