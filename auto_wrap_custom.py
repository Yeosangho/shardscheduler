...
def wrap(adaptive_sdp, module: nn.Module, **wrap_overrides: Any) -> nn.Module:
    """
    ...
    if ConfigAutoWrap.in_autowrap_context:
        ...
        print(adaptive_sdp)
        if(adaptive_sdp['FSDP'] > 0):
            adaptive_sdp['FSDP'] -= 1
            return FSDP(module, **wrap_overrides) 
        if(adaptive_sdp['DP'] > 0):
            adaptive_sdp['DP'] -= 1
            return DP(module, **wrap_overrides) 
        if(adaptive_sdp['SDP'] > 0):
            adaptive_sdp['SDP'] -= 1
            return SDP(module, **wrap_overrides)                              
    return module




def auto_wrap(adaptive_sdp, module: nn.Module, auto_wrap_policy: Optional[Callable] = None, **kwargs: Any) -> nn.Module:
    """
...
class ConfigAutoWrap:
    ...
    @staticmethod
    def recursive_wrap(adaptive_sdp, name, module: nn.Module, auto_wrap_policy: Optional[Callable], **kwargs: Any) -> Tuple[nn.Module, int]:
        """
