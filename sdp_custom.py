...
class ShardedDataParallel(nn.Module):
    ...
    def __init__(
        ...
        self._sdp_wrapped_module: nn.Module = FlattenParamsWrapper(module, param_list=to_be_flatten_params)
        ...
        # consolidation.
        self.params = cast(List[Parameter], self._sdp_wrapped_module.flat_params) + non_flatten_params




        self._num_flatten_params = len(self._sdp_wrapped_module.flat_params)
    ...
    def set_gradient_divide_factors(self, pre: float, post: float, recursive: bool) -> None:
        ...
        if recursive:
            for module in self.modules():
                if isinstance(module, ShardedDataParallel) and module != self:
    ...
    def module(self) -> FlattenParamsWrapper:
        ...
        assert isinstance(self._sdp_wrapped_module, FlattenParamsWrapper)
        return self._sdp_wrapped_module
    ...
    def _cast_buffers(
        ...
        if memo is None:
            memo = set()
        for module in self.modules():
            if module is not self and isinstance(module, ShardedDataParallel):
    ...
    def local_state_dict(self, *args: Any, **kwargs: Any) -> Any:
        ...
        with contextlib.ExitStack() as stack:
            ...
            return ShardedDataParallel.state_dict(self, *args, **kwargs)
    ...
    def no_sync(self) -> Generator:
        ...
        for m in self.modules():  # includes self
            if isinstance(m, ShardedDataParallel):
    ...
    def _set_is_root(self) -> None:
        ...
        for n, m in self.named_modules():
            # `n != ""` excludes self.
            if n != "" and isinstance(m, ShardedDataParallel):
    ...
    def _wait_unlock(self, lock, condition):
        ...
        if not lock.locked():
            ...
            with condition :
                #print("!!!!!!!locked!!!")
                #count = 0
                #for param in self._locks['AG']:
                #    print(f"{count} {self._locks['AG'][param]}")
                #    count += 1                
                condition.wait()
    ...
    def _fsdp_instances(self) -> List[nn.Module]:
        ...
        return [m for m in self.modules() if isinstance(m, ShardedDataParallel)]
...
def _post_state_dict_hook(
    module: ShardedDataParallel, state_dict: "OrderedDict[str, torch.Tensor]", prefix: str, *args: Any
    ...
    replace_by_prefix_(state_dict, prefix + "_sdp_wrapped_module.", prefix)
    return state_dict
...
def _pre_load_state_dict_hook(
    ...
    replace_by_prefix_(state_dict, prefix, prefix + "_sdp_wrapped_module.")


def _clean_path(path: str) -> str:
    """ Remove FSDP related wrapper modules from a given state dict key str path. """
    return ".".join([split for split in path.split(".") if split not in {"_sdp_wrapped_module", "_fpw_module"}])
...
def auto_wrap_bn(
    ...
    # Wrap it.
    with (
        enable_wrap(config_auto_wrap_policy, wrapper_cls=ShardedDataParallel) if wrap_it else contextlib.suppress()
    ):
